...
export default function SoundConfigModal({ visible, onClose }: SoundConfigModalProps) {
	const { bgMute, bgVolume, setBgMute, setBgVolume, sfxMute, sfxVolume, setSfxMute, setSfxVolume } = useSound();
		...
		<ModalOverlay>
			<SoundConfigContainer>
				...
				<SoundConfigSection>
					...
					<SoundConfigSlider id="bg-music-slider" value={Math.round(bgVolume * 100)} onChange={(e) => setBgVolume(Number(e.target.value) / 100)} />
					<SoundMuteButton onClick={() => setBgMute(!bgMute)}>{bgMute ? "Unmute" : "Mute"}</SoundMuteButton>
				</SoundConfigSection>
				...
				<SoundConfigSection>
					...
					/>
					<SoundMuteButton onClick={() => setSfxMute(!sfxMute)}>{sfxMute ? "Unmute" : "Mute"}</SoundMuteButton>
				</SoundConfigSection>
