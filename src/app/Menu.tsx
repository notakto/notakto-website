...
import { useRaiseCustomToast } from "@/components/hooks/useCustomToast";
const Menu = () => {
  const { raiseToast } = useRaiseCustomToast();
  ...
  const startGame = (mode: string) => {
    if ((mode === "liveMatch" || mode === "vsComputer") && !user) {
      if (canShowToast()) {
        raiseToast("Please sign in!", TOAST_IDS.User.SignInError);
      }
      return;
    }
    router.push(`/${mode}`);
